...
const RateModal = ({ isOpen, onClose, eventId, updateRateOnCard, previousRate }) => {
  ...
  const { postData } = useFetchAPI();
    ...
    const setRate = (star) => {
      ...
      postData('rates', rate).then((res) => {
        ...
        dispatch(addEventRate({eventId:eventId, averageScore: res.averageScore }))
      })
    }
    ...
    return (
        ...
        <div className="bg-white rounded-lg p-6 w-96">
          <h2 className="text-2xl mb-4">Rate Us</h2>
          <div className="flex justify-center mb-4">
            {[1, 2, 3, 4, 5].map((star) => (<Star key={star} filled={star <= rating} onClick={() => {setRating(star); setRate(star)}} previousRate={previousRate}/>
            ))}
          </div>
          <button onClick={() =>{ onClose(); updateRate();}} className="px-4 py-2 bg-blue-500 text-white rounded-md">
            Close
          </button>
        </div>
      </div>
